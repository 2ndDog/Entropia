...
train_continue = 309
if train_continue != 0:
    checkpoint_path = "model\\Entropia_temp-" + str(train_continue)
...
#--数据集----------------------------------------------------------------------------
with h5py.File("checkpoint\\train_temp.h5", "r") as hf:
...
# CNN5
layer_conv5 = tf.nn.conv2d(layer_conv4_shuffle,
                           weight_conv5,
                           strides=[1, 1, 1, 1],
                           padding="VALID")
...
learning_rate = tf.train.exponential_decay(
    1e-3, global_step, 50, 0.98, staircase=True) + 1e-4  #生成学习率

optimizer1 = tf.train.AdamOptimizer(learning_rate).minimize(
    l2, global_step=global_step)
    ...
    for ep in xrange(train_continue + 1, STEPS):
        ...
        for i in xrange(0,batch_step):
            ...
            if i % 50 == 0:
                ...
                with open("log/entropia_temp.txt", "a+") as f:
            ...
            if i == batch_step - 1:
                ...
                # 保存模型
                saver.save(sess, "model/Entropia_temp", global_step=ep)
