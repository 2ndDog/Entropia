...
merge_num_filters_conv2 = pre_num_filters_conv * 5+num_filters_conv1  # 最终合并层深度
...
def ResNet(input_layer,
    ...
    for l in range(layers):
        ...
        des_layer_conv = tf.nn.relu(des_layer_conv)
...
# CNN1非线性
layer_conv1=tf.nn.conv2d(pre_layer_conv,weight_conv1,strides=[1,1,1,1],padding="VALID")
...
layer_final = tf.nn.conv2d(layer_conv4_shuffle,
                           weight_conv5,
                           strides=[1, 1, 1, 1],
                           padding="VALID", name="output_image")
...
with tf.Session() as sess:
    ...
    for i in xrange(batch_step):
        ...
        SSIM, PSNR = sess.run([ssim, psnr],feed_dict=feed_dict_train)
        ...
        if i % 50 == 0:
            # 性能标准
            print("SSIM on dataset is %.8f \
            \nPSNR on dataset is %.6f" % (SSIM, PSNR))


    mean_ssim=np.mean(list_ssim)
    mean_psnr=np.mean(list_psnr)

    with open("log/index.txt", "a+") as f:
                    print("%.8f\t%.6f" % (mean_ssim, mean_psnr),file=f)
