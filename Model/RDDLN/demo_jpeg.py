...
PATH = ".\\checkpoint\\RDDLN_jpeg.pth"
...
def SuperResolution(path_image_input):
    ...
    for sub in range(math.ceil(len(list_sub_image_lr) / batch_size)):
        ...
        lr = (torch.Tensor(np.array(list_sub_image_lr[start:end]))).cuda().permute(0,3,1,2)
    ...
    image_result = np.zeros(
        (height * resolution - (low * resolution - high),
         width * resolution - (low * resolution - high), 3))
    for row in range(rows):
        for col in range(cols):
            # 坐标
            x0 = row * stride
            ...
            for h in range(h_offset, high):
                for w in range(w_offset, high):
                    for c in range(3):
                        image_result[result_x + h][result_y +
                                                   w][c] = result[h][w][c]
            ...
            for h in range(edge):
                for w in range(high):
                    for c in range(3):
                        image_result[result_x + h][result_y + w][c] = (
                            image_result[result_x + h][result_y + w][c] *
                            (1 - (h / edge))) + (result[h][w][c] * (h / edge))
...
path_image_input = ".\\.jpg"
path_image_output = ".\\.png"
